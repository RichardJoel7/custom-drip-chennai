-- Custom Drip Chennai — deduplicate customers by email, and stop it happening again
--
-- place_order() previously inserted a brand-new customers row on every single checkout,
-- even for a repeat buyer using the same email — so the same person could show up as
-- several unrelated-looking rows in Admin -> Customers. This migration:
--   1. Merges existing duplicate customer rows that share an email (case-insensitive),
--      keeping the earliest row and repointing any of its duplicates' orders to it.
--   2. Normalizes stored emails to lowercase/trimmed so the merge is meaningful.
--   3. Adds a unique index on customers.email so this can't happen again at the DB level.
--   4. Redefines place_order() to upsert the customer by email instead of always inserting.
--
-- Safe to run on a database with zero, one, or many existing duplicate customers.

-- 1. Repoint orders from duplicate customer rows to the earliest (canonical) row per email.
with ranked as (
...
canonical as (
  select dup.id as duplicate_id, keep.id as keeper_id
  from ranked dup
  join ranked keep on dup.email_norm = keep.email_norm and keep.rn = 1
  where dup.rn > 1
)
update orders o
set customer_id = canonical.keeper_id
from canonical
where o.customer_id = canonical.duplicate_id;

-- 2. Remove the now-orphaned duplicate customer rows.
with ranked as (
  select id, lower(trim(email)) as email_norm,
         row_number() over (partition by lower(trim(email)) order by created_at asc) as rn
  from customers
...
-- 3. Normalize remaining emails so the unique index below is meaningful.
...
-- 5. Reuse an existing customer by email instead of always inserting a new one.
...
begin
  ...
  insert into customers (full_name, mobile_number, email, instagram_username)
    values (p_full_name, p_mobile_number, v_email, nullif(trim(p_instagram_username), ''))
