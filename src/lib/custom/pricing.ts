...
// Mirrors place_order() in 0009_custom_studio.sql, which is what's actually charged.
export function customUnitPrice(config: CustomTeeConfig, catalog: CustomCatalog): number | null {
  ...
  return printPrice === null ? null : Number(size.price) + printPrice;
}
...
export function customCartKey(config: CustomTeeConfig) {
  ...
  return ["custom", config.colorId, config.sizeId, config.printOptionId, config.sides, front, back].join(":");
}
